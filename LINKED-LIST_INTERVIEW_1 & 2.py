...
class LinKed_List:
    ...
    def Print_LL(self, head): #hàm in giá trị của linked list
        # Current_Node = self.head #đặt biến chạy là head
        list = [] #tạo mảng rỗng để chứa data
        while (head): #duyệt hết list cho đến khi gặp None
            list.append(head.data) #truyền data vào mảng list
            head = head.next #di chuyển next đến next khác
    ...
    def FindMiddle(self):
        slowNode = self.head #đặt con trỏ cho slowNode
        fastNode = self.head #đặt con trỏ cho fastNode
        while fastNode and fastNode.next: #điều kiện để fastNode đi đến điểm cuối mà không trỏ vào NoneType
            slowNode = slowNode.next #di chuyển slowNode
            fastNode = fastNode.next.next #di chuyển fastNode
        return slowNode.data #trả về giá trị của Node giữa
    
    def Reversed_LL(self):
        
        prev, CurrentNode = None, self.head #đặt biến Prev và CurrentNode làm pointer trỏ vào 2 đầu

        while (CurrentNode): #điều kiện để pointer quét hết từ Node đầu đến Node cuối Linked-List
            #Các dòng code bên dưới là kĩ thuật để đổi 2 biến trong lập trình
            temp = CurrentNode.next 
            CurrentNode.next = prev
            prev = CurrentNode #đến dòng này là vì prev cần được lấy data từ CurrentNode
            CurrentNode = temp #trả lại Node bằng chính temp
        return prev #lúc này prev và head đã tráo đổi hay prev là head nên trả về từ head là đang chạy ngược lại
...
Link1.Print_LL(Link1.reversed_LL())

# print(Link1.FindMiddle())
